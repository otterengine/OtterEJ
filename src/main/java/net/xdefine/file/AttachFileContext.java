...
public class AttachFileContext {
	...
	public File getFile(IAttachIndex index) {
		return new File(XFContext.getProperty("webapp.file.path") + "/" + index.getSavedPath() + "/source");
	}
	
	public File getFile(IAttachIndex index, long width, long height, String type) {

		File tFile = new File(XFContext.getProperty("webapp.file.path") + "/" + index.getSavedPath() + "/" + width + "x" + height + "_" + type + ".jpg");
		if (tFile.exists()) return tFile;

		File afile = new File(XFContext.getProperty("webapp.file.path") + "/" + index.getSavedPath() + "/source");

		try {
			if (XFContext.getProperty("webapp.file.imagemagick") != null && !XFContext.getProperty("webapp.file.imagemagick").isEmpty()) {
				if (System.getProperty("os.name").contains("indows")) {
		            String[] cmd = new String[] { "\"" + XFContext.getProperty("webapp.file.imagemagick") + "\"", afile.toString(), "-resize", width + "x" + height + "^", "-quality", "100", "-gravity", "center", "-crop", width + "x" + height + "+0+0", "+repage", tFile.getPath() };
		            Process process = Runtime.getRuntime().exec(cmd);
		            process.waitFor();
				}
				else {
		            String[] cmd = new String[] { "/bin/sh", "-c", XFContext.getProperty("webapp.file.imagemagick") + " " + afile.getPath() + " -resize " + width + "x" + height+ "^ -quality 100 -gravity center -crop " +  width + "x" + height + "+0+0 +repage" + " " + tFile.getPath() };
		            Process process = Runtime.getRuntime().exec(cmd);
		            process.waitFor();
				}
			}
	...
	private File getFileForGD(IAttachIndex index, long width, long height, String thumbnail_type) {
		...
		File tFile = new File(XFContext.getProperty("webapp.file.path") + "/" + index.getSavedPath() + "/" + width + "x" + height + "_" + thumbnail_type + ".jpg");

		try {
			
			File file = new File(XFContext.getProperty("webapp.file.path") + "/" + index.getSavedPath() + "/source");
